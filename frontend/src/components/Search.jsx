...
users.filter(
    ...
    useEffect(() => {
	axios.post("http://localhost:5000/sendUsers" , {} , { withCredentials : true })
	.then(response =>{
	...
	<div className="bg-white rounded-xl shadow-2xl w-[300px] overflow-hidden">
		    ...
		    <X size={24} className="cursor-pointer hover:text-black"
			onClick={()=>{
				setMessageBox(false)
			    }}
		    />
            ...
            <div className="bg-white rounded-xl shadow-2xl w-[300px] overflow-hidden">
                ...
                <div className="max-h-96 overflow-y-auto">
                    {filteredUsers.length > 0 ? (
                            ...
                            {filteredUsers.map((user , index) => (
                                ...
                                >
                                    <div className="flex flex-col justify-start max-w-[150px]">
                                        <img
                                            src={(user.profileImageUrl === "/images/user.png") ? "https://api.dicebear.com/7.x/avataaars/svg?seed=Alice" : user.profileImageUrl}
                                            alt={user.fullName}
                                    ...
                                    <button className="p-4 rounded-xl bg-indigo-600 text-white cursor-pointer"
					    onClick = {()=>{
						axios.post("http://localhost:5000/connectUser" , {connectionId : user.id.toString()}, {withCredentials: true})
						.then(response =>{
							setMessageBox(true)
						...
						.catch(error =>{
							if(error.response.status === 403) navigation("/login")
							setMessageBox(true)
